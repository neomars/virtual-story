  ...
  <div class="user-management">
    <div class="header-container">
      ...
      <Transition name="fade">
        <div v-if="statusMessage" class="status-banner" :class="{ 'success': isSuccess }" role="alert">
          {{ statusMessage }}
        </div>
      </Transition>
    </div>
...
const createUser = async () => {
  ...
  try {
    const createdUsername = newUser.value.username;
    await axios.post('/api/admin/users', newUser.value);
    showStatus(`User "${createdUsername}" created successfully!`, true);
