  ...
  <div class="user-management">
    <div class="header-container">
      ...
      <Transition name="fade">
        <div v-if="statusMessage" class="status-msg" :class="{ success: isSuccess }" role="alert">
          {{ statusMessage }}
        </div>
      </Transition>
    </div>
...
const changePassword = async () => {
  ...
  try {
    ...
    showStatus(err.response?.data?.message || 'Failed to change password.', false);
  } finally {
...
const createUser = async () => {
  ...
  try {
    ...
    showStatus(err.response?.data?.message || 'Failed to create user.', false);
  } finally {
...
const deleteUser = async (user) => {
  if (confirm(`Delete user "${user.username}"?`)) {
    ...
    try {
      ...
      showStatus(err.response?.data?.message || 'Deletion failed.', false);
    } finally {
