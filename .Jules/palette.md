...
## 2025-06-02 - Hierarchical Complexity Management in Narrative Graphs
**Learning:** Interactive narrative editors with deep tree structures quickly become overwhelming. Implementing Expand/Collapse functionality at the node level allows users to focus on specific story branches while maintaining context. For accessibility, these toggles must use `aria-expanded` and include the target's title in their `aria-label` to provide sufficient context for screen reader users.
**Action:** Implement standardized expand/collapse toggles for all hierarchical tree components, ensuring proper ARIA attribute linkage and focus-visible indicators.
