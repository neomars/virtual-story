...
## 2025-05-28 - Recursive Tree Expansion in Administrative Graphs
**Learning:** In complex hierarchical narrative structures, users need the ability to focus on specific branches while hiding others to manage cognitive load. Providing a standard expand/collapse toggle at each node significantly improves navigability. Using `v-show` instead of `v-if` for these toggles ensures that the child container's `id` (targeted by `aria-controls`) remains in the DOM, maintaining accessibility.
**Action:** Use a reactive `isExpanded` ref and `v-show` for conditional rendering of child branches in recursive components, complemented by a toggle button with `aria-expanded` and `aria-controls`.
