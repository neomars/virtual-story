...
## 2025-06-01 - Reusable UX Pattern: Expandable Tree Branches
**Learning:** In recursive administrative interfaces like story graphs, providing the ability to collapse and expand branches improves information density management. Using a consistent `.toggle-btn` with a rotating arrow icon and a `.toggle-spacer` for leaf nodes ensures visual alignment of hierarchy and reduces cognitive load in large structures.
**Action:** Implement expandable branches using `v-show` (to keep `aria-controls` targets in DOM) and alignment spacers in all recursive tree-like UIs.
