...
## 2025-05-27 - Recursive Tree Navigation and Hierarchy Management
**Learning:** In recursive administrative graphs, users need the ability to manage screen real estate. Implementing collapse/expand toggles with `v-show` instead of `v-if` ensures that the targets for `aria-controls` remain in the DOM, preserving the accessibility relationship even when hidden.
**Action:** Use `v-show` for collapsible tree branches to maintain a stable DOM for screen readers, and provide a fixed-size `.toggle-spacer` for leaf nodes to ensure perfect vertical alignment of titles.
