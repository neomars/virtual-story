...
**Learning:** In applications managing complex hierarchical data (like a narrative graph), users benefit from the ability to expand and collapse branches to manage visual density. Providing these controls with proper ARIA attributes (`aria-expanded`) and visual feedback (rotating arrows) makes the interface both more powerful and more accessible.
**Action:** Implement collapsible branch logic in recursive tree components, ensuring leaf nodes are visually aligned with branch headers using appropriate spacing.
