...
## 2025-05-28 - Performance vs. Accessibility in Recursive Trees
**Learning:** In highly recursive UI structures like narrative graphs, using `v-show` for expandable branches preserves DOM nodes for ARIA relationships (like `aria-controls`) but can lead to severe performance degradation as the graph grows. `v-if` is the safer default for large administrative trees to keep the DOM lean and the interface responsive.
**Action:** Prioritize `v-if` for recursive branch expansion in large data sets, even if it means `aria-controls` targets are temporarily missing from the DOM when collapsed.
