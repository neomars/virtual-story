...
## 2025-05-27 - Recursive Tree Disclosure Patterns
**Learning:** In deeply nested narrative structures, visual clutter can quickly overwhelm administrative users. Implementing a per-node expansion toggle (disclosure) with persistent state allows users to focus on specific story branches. Using `v-show` instead of `v-if` for the toggleable content ensures that the `aria-controls` relationship remains valid in the DOM even when collapsed.
**Action:** Always provide expansion/collapse toggles for recursive tree components, and use `v-show` to maintain DOM presence for accessibility associations.
